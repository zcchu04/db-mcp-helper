...
- macOS：双击 `.dmg` 挂载后把 `Oracle MCP Setup` 拖入「应用程序」文件夹；首次启动如被 Gatekeeper 拦截，在「系统设置 → 隐私与安全性」里点「仍要打开」
- Linux：`.deb` 用 `sudo dpkg -i oracle-mcp-setup_0.1.0-1_amd64.deb`；app-image 解压后给 `Oracle MCP Setup` 加可执行位：`chmod +x "Oracle MCP Setup/Oracle MCP Setup"`

## 一、启动与向导

- [ ] 启动后浏览器自动打开 `http://127.0.0.1:8765/`（macOS/Linux 默认同 Windows）
- [ ] 向导首屏「部署运行时」默认根目录为 `~/.agent/mcp/oracle`，可改
- [ ] 点击「开始部署」后，根目录出现 `oracle-db-mcp-toolkit-1.0.0.jar`、`mcp-tap.jar`、`runtime/`、`state.json`
- [ ] `runtime/bin/java`（Linux）或 `runtime/bin/java`（mac）可执行，运行 `-version` 输出 17.x

## 二、加环境 + 自检 + 注册

- [ ] 添加一个真实环境（粘贴 Spring 配置片段或手工填写 host/port/service/user/password）
- [ ] 「保存并自检」返回 `ok:true`，数据库版本/延迟/用户名正确显示
- [ ] 「注册」后 `~/.qoderwork/mcp.json` 出现 `oracle-<env>` 条目，`command` 指向安装目录内的 `runtime/bin/java`
- [ ] 重启 QoderWork（或对该连接器关闭再开启）后，在对话里说「在 <env> 跑 db-ping」能成功
- [ ] 该环境的 `calllog.jsonl` 出现 db-ping 条目

## 三、多环境与权限隔离

- [ ] 添加第二个只读环境（不勾 `write-query`），注册后其 `args` 中不含 `write-query`
- [ ] 在只读环境上让 AI 跑 `write-query` → 报「工具不存在」
- [ ] 删除第一个环境后，mcp.json 条目消失、环境目录进回收站、Skill 映射表同步移除

## 四、接入指南

- [ ] 管理台某环境行点「接入指南」，平台列表出现：QoderWork / Claude Code / Cursor / Codex CLI / Gemini CLI / 其他
- [ ] 切换每个平台，模板预填的命令/JSON/TOML 中包含真实的 host/port/service 与 `runtime/bin/java` 路径
- [ ] 「复制配置模板」按钮可用，粘贴到对应客户端后能注册成功
- [ ] QoderWork 平台下「打开配置页」能跳转到 `qoder-work://connectors`

## 五、一键清空 / 卸载

- [ ] 一键清空：mcp.json 中的 oracle-* 条目消失、根目录进系统回收站、部署过的 Skill 副本也被同步移走
- [ ] 一键卸载：数据清空后程序目录在几秒内被移入回收站（macOS 用 `osascript` + Finder Trash；Linux 用 `gio trash` 或兜底 mv）
- [ ] 卸载后浏览器自动关闭（HTTP 服务已停）

## 六、Skill 同步

- [ ] Skill 部署到 `~/.qoderwork/skills` 后，在 QoderWork 对话里说「查 Oracle xxx」能触发 `oracle-db-ops` 技能
