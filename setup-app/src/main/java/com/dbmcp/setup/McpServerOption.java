 ...
 * @param toolkitResource 该实现对应的内置 toolkit 资源目录（classpath，如
 *                        {@code toolkit/mysql/mysql-mcp-server}）；null 表示用适配器级的
 *                        {@code toolkit/<srcDb>/<toolkitFileName>}。
 */
public record McpServerOption(
        ...
        String toolkitResource) {

    /** 无独立资源目录的便捷构造（适配器级 toolkit 即该实现的包体）。 */
    public McpServerOption(String id, String displayName, String description,
                           List<String> allTools, List<String> requiredTools) {
        this(id, displayName, description, allTools, requiredTools, null);
    }

    /** 兼容旧签名的便捷构造（allTools/requiredTools 为空清单，工具清单沿用适配器级定义）。 */
    public McpServerOption(String id, String displayName, String description) {
        this(id, displayName, description, List.of(), List.of(), null);
    }
}
